...
test("the fixed read commands are allowed exactly", () => {
  ...
  assert.equal(Command.allowed(Array.from(Command.DEVICES)), true)
  assert.deepEqual(Array.from(Command.DEVICES), ["hyprctl", "-j", "devices"])
  assert.equal(Command.allowed(["hyprctl", "-j", "devices", "all"]), false)
