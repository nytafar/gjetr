...
- Never edit `/usr/share/omarchy`. Never write the user's Config, except through `installConfig`, which writes only `gjetr.toml`, `layouts/<name>.toml` and their `.bak.<epoch>` backups under the Config dir (PRD M34). Validate `hyprctl reload` changes with `hyprctl configerrors`.
