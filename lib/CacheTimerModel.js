...
// The share of the ttl still left, 0 to 1, for a draining bar.
function fractionLeft(remaining, ttlSeconds) {
  if (!isInteger(ttlSeconds) || ttlSeconds <= 0 || typeof remaining !== "number" || !isFinite(remaining)) return 0
  return Math.max(0, Math.min(1, remaining / ttlSeconds))
}
...
function cacheTimer(agent, timers, now, settings) {
  ...
  return { remaining: remaining, level: name, label: formatRemaining(remaining, name), ttlSeconds: timer.ttlSeconds,
    fraction: fractionLeft(remaining, timer.ttlSeconds) }
}
