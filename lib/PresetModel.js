...
// A file's outcome when one of its writes failed: `kind` "backup" (the file
// was not replaced) or "file" (a backup already written is kept).
// -> { relative, action: "failed", backup, added, removed, line }
function failedOutcome(outcome, kind) {
  var o = outcome !== null && typeof outcome === "object" ? outcome : {}
  var name = String(o.relative)
  if (kind === "backup") {
    return { relative: name, action: "failed", backup: "", added: 0, removed: 0,
      line: name + ": not replaced, its backup could not be written" }
  }
  var backup = typeof o.backup === "string" ? o.backup : ""
  return { relative: name, action: "failed", backup: backup, added: 0, removed: 0,
    line: name + ": could not be written" + (backup !== "" ? " (backup " + backup + " kept)" : "") }
}

// What installing `files` (installFiles' result) into `configDir` writes, in
// order, given the text at each path now (`currentTexts`: path -> text, null
// when absent), and what each file's outcome is. A replaced file's backup comes
// before the file; an unchanged or refused file, or one whose backup may not be
// written, has no writes. Pure: the caller reads the current texts first, runs
// the writes and marks a file whose write fails with failedOutcome.
// -> { writes: [{ path, text, kind: "backup" | "file", relative }], outcomes, summaryLines }
function installPlan(configDir, files, currentTexts, epochSeconds) {
  var list = Array.isArray(files) ? files : []
  var current = currentTexts !== null && typeof currentTexts === "object" ? currentTexts : {}
  var writes = []
  var outcomes = []
  for (var i = 0; i < list.length; i++) {
    var file = list[i] !== null && typeof list[i] === "object" ? list[i] : {}
    if (!isInstallPath(configDir, file.path)) {
      outcomes.push({ relative: String(file.relative), action: "refused", backup: "", added: 0, removed: 0,
        line: String(file.relative) + ": refused" })
      continue
    }
    var before = Object.prototype.hasOwnProperty.call(current, file.path) ? current[file.path] : null
    var outcome = fileOutcome(file.relative, before, file.text, epochSeconds)
    if (outcome.action === "replaced") {
      var backupPath = String(configDir).replace(/\/+$/, "") + "/" + outcome.backup
      if (!isInstallPath(configDir, backupPath)) {
        outcomes.push(failedOutcome(outcome, "backup"))
        continue
      }
      writes.push({ path: backupPath, text: before, kind: "backup", relative: outcome.relative })
    }
    if (outcome.action !== "unchanged") writes.push({ path: file.path, text: file.text, kind: "file", relative: outcome.relative })
    outcomes.push(outcome)
  }
  return { writes: writes, outcomes: outcomes, summaryLines: outcomes.map(function(outcome) { return outcome.line }) }
}
