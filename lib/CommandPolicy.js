...
// Input devices, for the touchscreens gjetr looks for without a Config.
var DEVICES = ["hyprctl", "-j", "devices"]
// Where the pointer is, before a click on a Dock focuses a window.
var CURSOR_POS = ["hyprctl", "-j", "cursorpos"]
// Omarchy's usage collectors: regenerates ~/.local/state/omarchy/agents/usage.
// No arguments, so Config can never force, filter or add a collector.
var USAGE_UPDATE = ["omarchy-agent-usage-update"]
...
// Put the pointer back where it was. Hyprland's Lua config refuses the old
// `dispatch movecursor x y`; the dispatch is a Lua expression.
var CURSOR_MAX = 100000

function moveCursor(x, y) {
  if (typeof x !== "number" || typeof y !== "number" || !isFinite(x) || !isFinite(y) || Math.floor(x) !== x
    || Math.floor(y) !== y || Math.abs(x) > CURSOR_MAX || Math.abs(y) > CURSOR_MAX) return []
  return ["hyprctl", "dispatch", "hl.dsp.cursor.move({ x = " + x + ", y = " + y + " })"]
}

var CURSOR_RE = /^hl\.dsp\.cursor\.move\(\{ x = -?[0-9]{1,6}, y = -?[0-9]{1,6} \}\)$/
...
// merges into the output's existing one (finding T02), so every field
...
// Usage: list the records in Omarchy's usage directory. Only a directory that
// ends in omarchy/agents/usage, only its own *.json files, names only.
var USAGE_DIR_RE = /^\/[^\u0000-\u001f\u007f]*\/omarchy\/agents\/usage$/

function listUsageFiles(dir) {
  if (!isSafePath(dir, USAGE_DIR_RE)) return []
  return ["find", dir, "-maxdepth", "1", "-type", "f", "-name", "*.json", "-printf", "%f\\n"]
}

// Location: the repository and branch of an Agent's cwd (RepoModel). The cwd
// comes from herdr and is untrusted: absolute, no control or bidirectional
// characters, no parent steps. It is git's -C value, never an option.
var GIT_CWD_RE = /^\/[^\u0000-\u001f\u007f-\u009f\u200b-\u200f\u2028-\u202e\u2066-\u2069]{0,1023}$/
var GIT_ARGS = ["rev-parse", "--show-toplevel", "--abbrev-ref", "HEAD"]

function gitRepo(cwd) {
  if (!isSafePath(cwd, GIT_CWD_RE)) return []
  return ["git", "-C", cwd].concat(GIT_ARGS)
}

// Touch follows a runtime rotation only when told to (finding T09):
...
function allowed(argv) {
  ...
  if (same(argv, PROCESSES) || same(argv, CLIENTS) || same(argv, MONITORS) || same(argv, DEVICES) || same(argv, CURSOR_POS)
    || same(argv, USAGE_UPDATE)) return true
  if (argv.length === 3 && argv[0] === "hyprctl" && argv[1] === "dispatch" && FOCUS_RE.test(argv[2])) return true
  if (argv.length === 3 && argv[0] === "hyprctl" && argv[1] === "dispatch" && CURSOR_RE.test(argv[2])) return true
  ...
  if (argv[0] === "find") {
    return argv.length === 10 && (same(argv, locateTranscript(argv[1], argv[9].replace(/\.jsonl$/, "")))
      || same(argv, listUsageFiles(argv[1])))
  }
  if (argv[0] === "git") return argv.length === 7 && same(argv, gitRepo(argv[2]))
