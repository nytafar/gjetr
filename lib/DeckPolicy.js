...
// The Display that `state` and the IPC functions naming no Display describe:
// the first surface, else the first Display (a Config of Docks only). -1
// without Displays.
function primaryIndex(displays) {
  var list = Array.isArray(displays) ? displays : []
  for (var i = 0; i < list.length; i++) if (list[i] && list[i].kind !== "dock") return i
  return list.length > 0 ? 0 : -1
}

// Every Module across several Decks' active Layouts, in order. A Layout shown
// on two Displays gives the same keys twice; they are the same Modules, so
// each key is listed once.
function mergeModules(lists) {
  var source = Array.isArray(lists) ? lists : []
  var seen = {}
  var out = []
  for (var i = 0; i < source.length; i++) {
    var modules = Array.isArray(source[i]) ? source[i] : []
    for (var j = 0; j < modules.length; j++) {
      var module = modules[j]
      if (!module || typeof module.key !== "string" || module.key === "") continue
      if (Object.prototype.hasOwnProperty.call(seen, module.key)) continue
...
// The key of the first Module of `type` across Decks' Module lists, earlier
// lists first; "" when there is none.
function firstModuleKey(lists, type) {
  var source = Array.isArray(lists) ? lists : []
  for (var i = 0; i < source.length; i++) {
    var modules = Array.isArray(source[i]) ? source[i] : []
    for (var j = 0; j < modules.length; j++) {
      if (modules[j] && modules[j].type === type && typeof modules[j].key === "string" && modules[j].key !== "") return modules[j].key
    }
  }
  return ""
}
