...
//                        a dock: edge, size, visible
//   layouts/<name>.toml  orientation, [[module]] type, weight and its settings
//                        (agent-list: sort, preset, focus, recap, recap_open,
//                        highlight_workspace; workspace-list: tap, focus;
...
var COMMON_MODULE_KEYS = ["type", "weight"]
...
// maximum. Out of range clamps, with an error; anything else is unset (null).
function readRefresh(file, prefix, table, errors) {
  if (!own(table, "refresh_seconds")) return null
...
function readDisplay(file, index, table, errors) {
  ...
  // A mistyped kind or a dock without an edge would put a surface somewhere
  // unintended, so the Display is skipped rather than guessed.
  ...
  var dock = kind === "dock"
  if (dock && !DockPolicy.isEdge(table.edge)) {
    errors.push(file + ": " + prefix + "edge: a dock needs one of " + sameStrings(DockPolicy.EDGES) + ", got "
      + (own(table, "edge") ? show(table.edge) : "nothing") + "; display skipped")
    return null
  }
  unknownKeys(file, prefix, table, DISPLAY_KEYS.concat(SURFACE_KEYS, DOCK_KEYS), errors)
  ...
  display.refreshSeconds = readRefresh(file, prefix, table, errors)
...
function readAgentList(file, prefix, table, errors) {
  ...
  unknownKeys(file, prefix, table, COMMON_MODULE_KEYS.concat(["sort", "preset", "focus", "recap", "recap_open",
    "highlight_workspace"]), errors)
...
function readUsage(file, prefix, table, errors) {
  var settings = defaultUsage()
  unknownKeys(file, prefix, table, COMMON_MODULE_KEYS.concat(["show", "providers", "refresh_seconds"]), errors)
  settings.weight = readWeight(file, prefix, table, errors)
  settings.refreshSeconds = readRefresh(file, prefix, table, errors)
  if (own(table, "show")) {
    if (!Array.isArray(table.show)) {
      ...
      else errors.push(file + ": " + prefix + "show: no valid items, using " + show(UsageModel.DEFAULT_SHOW))
    }
  }
  if (own(table, "providers")) {
    if (!Array.isArray(table.providers)) {
      ...
      for (var j = 0; j < table.providers.length; j++) {
        ...
        } else if (settings.providers.indexOf(id) < 0 && settings.providers.length < UsageModel.MAX_PROVIDERS) {
          settings.providers.push(id)
        }
      }
    }
  }
...
function readWorkspaceList(file, prefix, table, errors) {
  var settings = defaultWorkspaceList()
  unknownKeys(file, prefix, table, COMMON_MODULE_KEYS.concat(["tap", "focus"]), errors)
  settings.weight = readWeight(file, prefix, table, errors)
  var scoped = []
  settings.tap = pick(file, "tap", table, WorkspaceTreeModel.TAP_MODES, settings.tap, scoped)
  settings.focus = pick(file, "focus", table, FOCUS_MODES, settings.focus, scoped)
  for (var i = 0; i < scoped.length; i++) errors.push(scoped[i].replace(file + ": ", file + ": " + prefix))
  return settings
}
...
function readLayout(name, text) {
  ...
  for (var i = 0; i < list.length; i++) {
    ...
    var reader = list[i].type === "workspace-list" ? readWorkspaceList
      : list[i].type === "usage" ? readUsage
      : readAgentList
    modules.push(reader(file, prefix, list[i], errors))
  }
