...
// gjetr writes Config only when asked to install a preset (PresetModel, IPC
// installConfig).
//
...
//   layouts/<name>.toml  orientation, [[module]] type, weight, pin (usage) and its
//                        settings
//                        (TYPE_KEYS: every type: density; agent-list: sort, preset, focus, recap,
//                        recap_open, highlight_workspace, indicator, working_effect;
//                        workspace-list: tap, focus, indicator, working_effect;
...
// A Display on no output: what a Config without Displays holds until gjetr has
// detected what to show (DetectPolicy). No screen has this name, so nothing is
// drawn.
var DEFAULT_DISPLAY = ""
...
var MAX_WEIGHT = 100
// A pinned Module sits at the end of its Layout, sized to its content
// (LayoutPolicy.moduleRects). Only a Usage Module has a content height to
// size to: the lists scroll.
var PINS = ["none", "end"]
var PIN_TYPES = ["usage"]
...
function defaultAgentList() {
  return { type: "agent-list", sort: SortPolicy.DEFAULT_MODE, preset: CardPolicy.DEFAULT_PRESET, focus: "herdr",
    ...
    density: DensityPolicy.DEFAULT_SETTING, indicator: IndicatorPolicy.DEFAULT_MODE,
    workingEffect: IndicatorPolicy.DEFAULT_WORKING_EFFECT }
}

function defaultUsage() {
  return { type: "usage", show: UsageModel.DEFAULT_SHOW.slice(), providers: [], refreshSeconds: null, weight: DEFAULT_WEIGHT,
    pin: "none", density: DensityPolicy.DEFAULT_SETTING }
}
...
// A copy of each detected Display, or null when there are none.
function detectedDisplays(list) {
  if (!Array.isArray(list) || list.length === 0) return null
  return list.map(function(display) { return copySettings(display) })
}

// text is null or undefined when the file does not exist. `detected`: the
// Displays gjetr detected (DetectPolicy), used when the file gives none.
function readMain(text, home, detected) {
  var file = "gjetr.toml"
  var errors = []
  var config = defaults(home)
  var fallback = detectedDisplays(detected)
  if (fallback) config.displays = fallback
  ...
  if (own(table, "display")) {
    ...
    if (!list) {
      ...
      else errors.push(file + ": no valid display, using " + (fallback ? "the detected Displays" : "none until one is detected"))
    }
  }
...
var MODULE_ONLY_KEYS = ["type", "weight", "pin"]
// The settings of each Module type, which [defaults] and [defaults.<type>]
// may give as well as a [[module]].
var TYPE_KEYS = {
  "agent-list": ["density", "sort", "preset", "focus", "recap", "recap_open", "highlight_workspace", "indicator",
    "working_effect"],
  "workspace-list": ["density", "tap", "focus", "indicator", "working_effect"],
...
// The kind mark as status indicator (IndicatorPolicy), for both lists.
function applyIndicator(file, prefix, table, settings, errors) {
  settings.indicator = pick(file, prefix, "indicator", table, IndicatorPolicy.MODES, settings.indicator, errors)
  settings.workingEffect = pick(file, prefix, "working_effect", table, IndicatorPolicy.WORKING_EFFECTS,
    settings.workingEffect, errors)
}
...
function applyAgentList(file, prefix, table, settings, errors) {
  ...
  applyIndicator(file, prefix, table, settings, errors)
}
...
function readModule(file, prefix, table, base, errors) {
  ...
  if (PIN_TYPES.indexOf(table.type) >= 0) {
    settings.pin = pick(file, prefix, "pin", table, PINS, settings.pin === "end" ? "end" : "none", errors)
  } else if (own(table, "pin")) {
    errors.push(file + ": " + prefix + "pin: only a Usage Module sizes to its content (ignored)")
  }
...
function readLayout(name, text, defaults) {
  ...
  if (text === undefined || text === null) {
    errors.push(file + ": not found in the Config or among gjetr's Layouts, using the default Agent List")
...
// Each Layout's text: the Config's layouts/<name>.toml when it has one, else the
// one gjetr ships in presets/layouts/. A text is null while the file does not
// exist and absent while it has not answered yet, for either. A Layout waits
// for the Config's answer before a shipped one stands in.
// -> { texts: { name: text or null }, sources: { name: "config" | "shipped" | "missing" | "loading" } }
function overlayLayouts(names, userTexts, shippedTexts) {
  var list = Array.isArray(names) ? names : []
  var user = isTable(userTexts) ? userTexts : {}
  var shipped = isTable(shippedTexts) ? shippedTexts : {}
  var texts = {}
  var sources = {}
  for (var i = 0; i < list.length; i++) {
    var name = list[i]
    if (!isLayoutName(name)) continue
    var mine = own(user, name) ? user[name] : undefined
    var theirs = own(shipped, name) ? shipped[name] : undefined
    if (typeof mine === "string") {
      texts[name] = mine
      sources[name] = "config"
    } else if (mine === null && typeof theirs === "string") {
      texts[name] = theirs
      sources[name] = "shipped"
...
function layoutModules(layout) {
  ...
  for (var i = 0; i < modules.length; i++) {
    ...
    out.push({ index: i, key: name === "" ? "" : name + "#" + i, type: modules[i].type,
      weight: modules[i].weight, pin: modules[i].pin === "end" ? "end" : "none", settings: modules[i] })
  }
  return out
}
