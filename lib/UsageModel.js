...
// (docs/adr/0002-usage-behind-a-source-seam.md). Pure.
//
// Provider:
...
//     cost30d: { amount, currency } or null when the Source has no cost }
//
// Records are untrusted: numbers are checked, text is cleaned and capped.

var SHOW_ITEMS = ["limits", "today", "recent_days", "models", "cost_30d"]
...
function fromOmarchy(record, fileId) {
  ...
  return {
    ...
    // Omarchy's records carry no cost, and gjetr does not price tokens itself.
    cost30d: null
  }
}
...
function placeholder(id) {
  return { id: id, source: "", name: id, tier: "", ready: false, statusText: "no usage data", updatedAtMs: 0, limits: [],
    today: { tokens: 0, prompts: 0, sessions: 0, hasPrompts: false }, recentDays: [], models: [], cost30d: null }
}
...
function formatMoney(cost) {
  if (!isObject(cost) || typeof cost.amount !== "number" || !isFinite(cost.amount) || cost.amount < 0) return ""
  var currency = /^[A-Z]{3}$/.test(str(cost.currency)) ? cost.currency : "USD"
  return (currency === "USD" ? "$" : currency + " ") + cost.amount.toFixed(2)
}
