...
var STATUS_TONES = { blocked: "urgent", done: "accent", working: "foreground", idle: "muted", unknown: "muted" }
...
function cardHeightFor(preset, recapInline) {
  return cardHeight(preset) + (recapInline ? RECAP_INLINE_PX : 0)
}

function statusTone(status) {
  return own(STATUS_TONES, status) ? STATUS_TONES[status] : "muted"
}
