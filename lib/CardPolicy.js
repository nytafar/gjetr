...
// The kind mark: Omarchy's SVG or gjetr's one-colour SVG where the kind has
// one, else the letter drawn in its place (KindPolicy).
function kindIconFile(kind, lightTheme) {
  return KindPolicy.kindIconFile(kind, lightTheme)
}

function kindIconTinted(kind) {
  return KindPolicy.kindIconTinted(kind)
}

function kindIconUrl(kind, lightTheme, omarchyPath, assetsUrl) {
  return KindPolicy.kindIconUrl(kind, lightTheme, omarchyPath, assetsUrl)
}
